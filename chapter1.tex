...
En fait, le comportement électrique d'un telle cellule en l'absence de la lumière est identique à celui d'une diode PN classique dont la caractéristique est décrite par l'équation de Shockley (équation \ref{eq:shockley}). Dans cette formule, le $I_0$ \nomenclature{$I_0$}{Courant de Saturation \nomunit{\si{\micro\ampere}}} est le courant de saturation de la diode, $q$ \nomenclature{$q$}{Charge Élémentaire \nomunit{$1.6021764 \times 10^{-19}\ \si{\coulomb}$}} la charge élémentaire $e^{-}$, $a$ le facteur d'idéalité\nomenclature{$a$}{Facteur d'idéalité}, $k$ la constante de Boltzmann \nomenclature{$k$}{Constante de Boltzmann \nomunit{$1.3806503 \times 10^{-23}$ \si{\joule\per\kelvin}} } et $T$ \nomenclature[1temp]{$T$}{Température de la cellule/du module \nomunit{\si{\kelvin}}} la température de la cellule. Parfois on utilise la notion de \textit{voltage thermique}: $V_t = \frac{kT}{q}$ \nomenclature[1voltherm]{$V_t$}{Voltage Thermique \nomunit{\si{\volt}}}.
...
\nomenclature{$I_D$}{Courant de la diode \nomunit{\si{\ampere}}}
...
\nomenclature{$R_s$}{Résistance série \nomunit{\si{\ohm}}}
\nomenclature{$R_p$}{Résistance shunt \nomunit{\si{\ohm}}}
